...
public:
	...
	//多值函数wsqtd，获得实时行情
	//非订阅模式，取一次性快照数据
	static LONG wsqtd(WindData& outWindData, LPCWSTR windCodes, LPCWSTR fields, LPCWSTR options = NULL);

	//订阅模式td，订阅实时数据，数据通过回调函数返回
	static LONG wsqtd(ULONGLONG &reqId, LPCWSTR windCodes, LPCWSTR fields, WsqCallBack callBack, LPCWSTR options = NULL, BOOL updateAllFields = FALSE);
