...
void CWindQuantCppSampleDlg::OnBnClickedButton1()
{
	...
	else
	{
		WCHAR buffer[128];
		int bufferSize = 128;
		WindCppAPI::getErrorMsg(errCode, eCHN, buffer, bufferSize);
...
void CWindQuantCppSampleDlg::OnBnClickedButton3()
{
	...
	case 0://wsd
		{
			errCode = WindCppAPI::wsd(wdata, m_windCodes, m_indicators, m_startTime, m_endTime);
			if (errCode == 0)
			{
	...
	case 1://wss
		{
			errCode = WindCppAPI::wss(wdata, m_windCodes, m_indicators, m_options);
			if (errCode == 0)
			{
				updateList(wdata);
			}
	...
	case 2://wsi
		{
			errCode = WindCppAPI::wsi(wdata, m_windCodes, m_indicators, m_startTime, m_endTime, m_options);
			if (errCode == 0)
			{
				updateList(wdata);
			}
	...
	case 3://wst
		{
			errCode = WindCppAPI::wst(wdata, m_windCodes, m_indicators, m_startTime, m_endTime, m_options);
			if (errCode == 0)
			{
				updateList(wdata);
			}
	...
	case 4://wsq
		{
			...
			if (state == 0)
			{
				errCode = WindCppAPI::wsq(wdata, m_windCodes, m_indicators, m_options);
				if (errCode == 0)
				{
					updateList(wdata);
				}
			...
			{
				errCode = WindCppAPI::wsq(m_reqId, m_windCodes, m_indicators, CallBack, m_options, TRUE);
			}
	...
	case 5://wset
		{
			errCode = WindCppAPI::wset(wdata, m_reportName, m_options);
			if (errCode == 0)
			{
				updateList(wdata);
			}
	...
	case 6://wpf
		{
			errCode = WindCppAPI::wpf(wdata, m_portfolioName, m_viewName, m_options);
			if (errCode == 0)
			{
				updateList(wdata);
			}
	...
	case 7://tdays
		{
			errCode = WindCppAPI::tdays(wdata, m_startTime, m_endTime, m_options);
			if (errCode == 0)
			{
				initList();
				likeTDays(wdata);
	...
	case 8://tdayscount
		{
  			LONG count = 0;
			errCode = WindCppAPI::tdayscount(count, m_startTime, m_endTime, m_options);
			if (errCode == 0)
			{
				CString str;
	...
	case 9://tdaysoffset
		{
			DATE date = 0;
			errCode = WindCppAPI::tdaysoffset(date, m_startTime, m_offset, m_options);
			if (errCode == 0)
			{
...
void CWindQuantCppSampleDlg::OnBnClickedCheckSub()
{
	...
	{
		WindCppAPI::cancelRequest(m_reqId);
		m_reqId = 0;
	}
}
